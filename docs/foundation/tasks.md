...
**Implementation status:** `FND-BST-001` through `FND-BST-005` and `FND-SHL-001` complete; proposed `FND-SHL-001A` awaits approval and is not authorized for implementation; `FND-BST-006` and all other tasks remain pending.
...
| `FND-SHL-001A` | SHL-001, BST-005 | Bounded hostname domain value/read port, Linux `gethostname(2)` adapter, application use case, generated `SystemIdentity` contract, narrow core Tauri/API query, and sparse host-aware Dashboard with no shortcuts/onboarding/metrics. | Value/source/use-case/contract/Tauri/API/Dashboard loading-success-unavailable/direct-route tests; update actual identity source and UI layout. **Done when Dashboard identifies the machine without feature fetch/polling and another directly opened core route performs no hostname read. Proposed; implementation requires approval.** |
